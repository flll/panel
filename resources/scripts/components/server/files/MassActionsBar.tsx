...
const MassActionsBar = () => {
    ...
    const onClickCompress = () => {
        setLoading(true);
        clearFlashes('files');
        setLoadingMessage('ファイルをアーカイブしています...');
    ...
    const onClickConfirmDeletion = () => {
        ...
        clearFlashes('files');
        setLoadingMessage('ファイルを削除しています...');
        ...
        <>
            <div css={tw`pointer-events-none fixed bottom-0 z-20 left-0 right-0 flex justify-center`}>
                ...
                >
                    <p className={'mb-2'}>
                        本当に&nbsp;
                        <span className={'font-semibold text-gray-50'}>{selectedFiles.length} ファイル</span>を削除しますか？これは
                        永久的な操作で、ファイルは復元できません。
                    </p>
                    {selectedFiles.slice(0, 15).map((file) => (
                        <li key={file}>{file}</li>
                    ))}
                    {selectedFiles.length > 15 && <li>その他{selectedFiles.length - 15}個</li>}
                </Dialog.Confirm>
                {showMove && (
                    <RenameFileModal
                        files={selectedFiles}
