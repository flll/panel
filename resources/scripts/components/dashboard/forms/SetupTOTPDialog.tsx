        ...
        <form id={'enable-totp-form'} onSubmit={submit}>
            ...
            <CopyOnClick text={token?.secret}>
                <p className={'font-mono text-sm text-gray-100 text-center mt-2'}>
                    {token?.secret.match(/.{1,4}/g)!.join(' ') || '読み込み中...'}
                </p>
            </CopyOnClick>
            <p id={'totp-code-description'} className={'mt-6'}>
                上のQRコードをお選びの二段階認証アプリでスキャンしてください。その後、以下のフィールドに生成された6桁のコードを入力してください。
            </p>
            <Input.Text
            ...
            <Dialog.Footer>
                ...
                <Tooltip
                    ...
                    content={
                        !token
                            ? 'QRコードの読み込みを待っています...'
                            : '続行するには6桁のコードとパスワードを入力する必要があります。'
                    }
                    delay={100}
                >
                    <Button
...
export default asDialog({
    title: '二段階認証を有効にする',
    description:
        "アカウントへの不正アクセスから保護するために、サインインするたびに確認コードが求められます。",
