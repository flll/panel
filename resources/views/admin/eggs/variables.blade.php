                ...
                <form action="{{ route('admin.nests.egg.variables.edit', ['egg' => $egg->id, 'variable' => $variable->id]) }}" method="POST">
                    <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                <p class="text-muted small">この変数は、起動コマンドで<code>{{ $variable->env_variable }}</code>を使用してアクセスできます。</p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">権限</label>
                            <select name="options[]" class="pOptions form-control" multiple>
                                <option value="user_viewable" {{ (! $variable->user_viewable) ?: 'selected' }}>ユーザーが表示可能</option>
                                <option value="user_editable" {{ (! $variable->user_editable) ?: 'selected' }}>ユーザーが編集可能</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form-label">入力ルール</label>
                            <input type="text" name="rules" class="form-control" value="{{ $variable->rules }}" />
                            <p class="text-muted small">これらのルールは、標準の<a href="https://laravel.com/docs/5.7/validation#available-validation-rules" target="_blank">Laravelフレームワークのバリデーションルール</a>を使用して定義されます。</p>
                        </div>
                    </div>
            ...
            <form action="{{ route('admin.nests.egg.variables', $egg->id) }}" method="POST">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12">
                            <p class="text-muted small">この変数は、起動コマンドで<code>@{{環境変数の値}}</code>を入力することでアクセスできます。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label">権限</label>
                        <select name="options[]" class="pOptions form-control" multiple>
                            <option value="user_viewable">ユーザーが表示可能</option>
                            <option value="user_editable">ユーザーが編集可能</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="control-label">入力ルール <span class="field-required"></span></label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">変数を作成</button>
                </div>
            </form>
        </div>
    </div>
